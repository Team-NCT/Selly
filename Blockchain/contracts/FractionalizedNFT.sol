...
contract FractionalizedNFT is ERC20, Ownable, ERC20Permit, ERC721Holder {
    ...
    event StartAuc(address F_CA, uint256 minimumPrice, bool auctionStatus);
    ...
    function createSale(uint256 _amount, uint256 _price)
        public
        returns (address)
    {
    ...
    function startAuction(uint256 _startPrice) external {
        require(
            ((100 * balanceOf(msg.sender)) / totalSupply()) > 50,
            "Only user who own more than 50% of fractions can start auction"
        );
    ...
    function cancelAuction() external {
        require(
            ((100 * balanceOf(msg.sender)) / totalSupply()) > 50,
            "Only user who own more than 50% of fractions can cancel auction"
        );
        require(!ended, "Auction Ended");
        require(highestBid == 0, "there is someone who makes a bid");
    ...
    function bid() external payable {
        ...
        require(
            highestBid == 0 || block.timestamp <= auctionEndTime,
            "Not to auction"
        );
        require(
            msg.value >= minimumPrice,
            "Have to Bid higher than minimumPrice"
        );
